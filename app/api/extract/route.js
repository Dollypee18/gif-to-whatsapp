...
function pickBestMp4(tweetData) {
  ...
  const mediaDetails =
    tweetData?.mediaDetails || tweetData?.extended_entities?.media || [];
...
export async function POST(request) {
  ...
  if (!tweetId) {
    return Response.json(
      {
        error:
          "Couldn't find a tweet ID in that URL. Make sure it looks like https://x.com/user/status/123...",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return Response.json(
      {
        error:
          "Couldn't reach X's media endpoint. This can mean the tweet is private/deleted, or X has changed/rate-limited the endpoint this relies on.",
